<?php
	...
	if ($_SESSION["oturum_acildi"] != true)
	{
		header("Location: giris.php");
		exit();
	}
...
<html lang="en">
  <head>
	...
	<body>
	<?php
		$sorgu = "SELECT * FROM `kullanici` WHERE `kullanici_adi`='".$_SESSION["kullanici_adi"]."'"; 
		$sonuc = $baglanti->query($sorgu);
		$yetki_seviyesi = $sonuc->fetch_row()[3];

	?>
	<h3><?=$_SESSION["kullanici_adi"]?> Hoşgeldiniz...</h3>
	<h3>Yetki Seviyeniz : <?=$yetki_seviyesi?></h3>
...
echo "<button type='submit' class='btn btn-info duzenleDugmesi' data-kayitno='".$musteri["id"]."'>Düzenle</button></td>";
	...
	<?php
		...
		{
			echo "Yetki seviyeniz müşteri eklemek için yeterli değildir...";
		}
	?>
